...
{
	/* 参数初始化 */
	phonecall_detect_result_group_t detect_result_group;

	/* 算法模型初始化 */
	rknn_context ctx;
	phonecall_detect_init(&ctx, "./phonecall_detect.model");
	...
	phonecall_detect_run(ctx, src, &detect_result_group);
	...
	{
		phonecall_detect_result_t *det_result = &(detect_result_group.results[i]);
