...
{
	/* 参数初始化 */
	site_safety_result_group_t detect_result_group;

	/* 算法模型初始化 */
	rknn_context ctx;
	site_safety_init(&ctx, "./site_safety.model");

	/* 算法运行 */
	cv::Mat src;
	src = cv::imread("test-1.jpg", 1);
	...
	site_safety_run(ctx, src, &detect_result_group);
	...
	{
		site_safety_result_t *det_result = &(detect_result_group.results[i]);
