...
{
	/* 参数初始化 */
	smoke_detect_result_group_t detect_result_group;

	/* 算法模型初始化 */
	rknn_context ctx;
	smoke_detect_init(&ctx, "./smoke_detect.model");
	...
	smoke_detect_run(ctx, src, &detect_result_group);
	...
	{
		smoke_detect_result_t *det_result = &(detect_result_group.results[i]);
